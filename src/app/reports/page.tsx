  ...
  return (
    <div className="space-y-6 max-w-7xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold text-foreground tracking-tight">Reports</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {[pCalc, sCalc].map((calc, i) => {
          ...
          return (
            <div key={label} className="rounded-lg border border-border bg-card p-3 md:p-4">
              <p className="text-xs text-muted-foreground mb-1">{label}</p>
              <p className="text-lg md:text-xl font-semibold text-foreground">{formatCurrency(calc.roomTotalBill)}</p>
          ...
          <table className="w-full text-xs md:text-sm">
            <thead>
              <tr className="bg-muted">
                <th className="text-left py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">House</th>
                <th className="text-left py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">Room</th>
                <th className="text-left py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs hidden sm:table-cell">Tenant</th>
                <th className="text-left py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs hidden md:table-cell">Type</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs hidden md:table-cell">Previous</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs hidden md:table-cell">Current</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">Units</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">Bill</th>
              </tr>
            </thead>
            <tbody>
              {allRooms.map((r, i) => (
                <tr key={i} className="border-t border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="py-2 px-3 md:px-4 text-muted-foreground hidden sm:table-cell">{r.name || <span className="italic">&mdash;</span>}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-border bg-muted/30">
                <td colSpan={6} className="py-2.5 px-3 md:px-4 text-right font-semibold text-foreground text-xs md:text-sm">Totals</td>
                <td className="py-2.5 px-3 md:px-4 text-right font-semibold text-foreground text-xs md:text-sm">
                  {formatNumber(allRooms.reduce((s, r) => s + r.units, 0))}
                </td>
                <td className="py-2.5 px-3 md:px-4 text-right font-semibold text-emerald-400 text-xs md:text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs md:text-sm">
            <thead>
              <tr className="bg-muted">
                <th className="text-left py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">House</th>
                <th className="text-left py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">Meter</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs hidden md:table-cell">Previous</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs hidden md:table-cell">Current</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">Units</th>
                <th className="text-right py-2.5 px-3 md:px-4 text-muted-foreground font-medium text-xs">Bill</th>
              ...
              {allExtraMeters.map((m, i) => (
                <tr key={i} className="border-t border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="py-2 px-3 md:px-4 text-right text-emerald-400 font-medium">
                    {m.type === "main" ? <span className="text-muted-foreground">&mdash;</span> : formatCurrency(m.bill)}
                  </td>
                </tr>
              ))}
            </tbody>
