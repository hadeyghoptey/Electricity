...
import { formatCurrency, formatNumber, getCurrentMonth, mapHouseRawData, computeMonthlyUsage } from "@/lib/utils";
    ...
    <div className="space-y-8 max-w-7xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            {month} &middot; Overall electricity summary
          </p>
        </div>
        <button
          onClick={refresh}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <div className="rounded-lg border border-border bg-card p-4 md:p-5">
          ...
          <p className="text-xl md:text-2xl font-semibold text-foreground">{formatCurrency(overallGrand)}</p>
          <p className="text-xs text-muted-foreground mt-1">Grand Total (incl. water)</p>
        </div>

        <div className="rounded-lg border border-border bg-card p-4 md:p-5">
          <p className="text-xs text-muted-foreground mb-1">Collection</p>
          <p className="text-xl md:text-2xl font-semibold text-emerald-400">{formatCurrency(overallRoom)}</p>
          <p className="text-xs text-muted-foreground mt-1">{formatNumber(overallUnits)} units</p>
        </div>

        <div className="rounded-lg border border-border bg-card p-4 md:p-5">
          <p className="text-xs text-muted-foreground mb-1">Main Meter</p>
          <p className="text-xl md:text-2xl font-semibold text-amber-400">{formatNumber(overallMain)}</p>
          <p className="text-xs text-muted-foreground mt-1">total units purchased</p>
        </div>

        <div className="rounded-lg border border-border bg-card p-4 md:p-5">
          <p className="text-xs text-muted-foreground mb-1">Water Bills</p>
          <p className="text-xl md:text-2xl font-semibold text-blue-400">{formatCurrency(overallExtra)}</p>
          <p className="text-xs text-muted-foreground mt-1">Khanepani + Melamchi</p>
        </div>
      </div>

      <UsageChart
        primaryData={computeMonthlyUsage(primary)}
        secondaryData={computeMonthlyUsage(secondary)}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        {[pCalc, sCalc].map((calc, i) => {
          ...
          return (
            <div key={label} className="rounded-lg border border-border bg-card p-4 md:p-5">
              ...
              <div className="grid grid-cols-2 gap-2 md:gap-3 mb-4">
                <div className="bg-muted rounded-lg p-3">
                  ...
                  <p className="text-base md:text-lg font-semibold text-emerald-400">{formatCurrency(calc.roomTotalBill)}</p>
                </div>
                <div className="bg-muted rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Units</p>
                  <p className="text-base md:text-lg font-semibold text-foreground">{formatNumber(calc.totalUnits)}</p>
                </div>
                <div className="bg-muted rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Main Meter</p>
                  <p className="text-base md:text-lg font-semibold text-amber-400">{formatNumber(calc.mainMeter?.units ?? 0)}</p>
                </div>
                <div className="bg-muted rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Loss</p>
                  <p className="text-base md:text-lg font-semibold text-red-400">{formatNumber(calc.lossPercent, 1)}%</p>
                </div>
              </div>

              <div className="border-t border-border pt-3 flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Grand Total</span>
                <span className="font-semibold text-foreground">{formatCurrency(calc.grandTotal)}</span>
              </div>
            </div>
          );
        })}
